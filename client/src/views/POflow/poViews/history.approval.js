...
export default function ApprovalHistory(props) {
  ...
  const delayFiltering = useCallback((currentData, filter) => {
    currentData.filteredContent = currentData.content.filter(
      (row) =>
        ...
        row.poData.poNumber.includes(filter) ||
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await API.getApprovalHistory()
  ...
  return (
    <div className="approvalHistory">
      <h5>Purchase Orders I Approved</h5>
              ...
              allData.filteredContent.map((row, i) => (
                <div key={i}>
                  <ModalButton
                    ...
                    btnText={
                      <tr className="row">
                        ...
                        <td className="col">{row.poData.poNumber}</td>
                  ...
                  <div className="trashcanbig">
                    {row.poData && row.poData.finance && (
                      <ModalButton
                        icon={editIcon}
                        width={25}
                        height={25}
                        alt="Edit Purchase Order."
                        status={row.header.stage}
                        form={POEditForm}
                        modalTitle="Edit existing purchase order."
                        updateDeleteInfo={{
                          id: row.header.formId,
                          shippingHandling: row.header.shippingHandling,
                          quote: row.header.quote,
